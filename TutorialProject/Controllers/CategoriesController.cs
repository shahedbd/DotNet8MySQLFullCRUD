...
namespace TutorialProject.Controllers
{
    public class CategoriesController : Controller
    {
        ...
        {
            var _Category = await _context.Category.OrderByDescending(x => x.Id).ToListAsync();
            if (_Category.Count < 1) await CreateTestData();
            return _context.Category != null ? View(_Category) : Problem("Entity set 'ApplicationDbContext.Category'  is null.");
        }
